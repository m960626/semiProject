		...
		<div class="subContainer-wrap">
			...
			<div class="subContainer">
				<div class="wrap">
					...
					<!-- 게시판 게시물 갯수 -->
					<div class="wrap tbl-total">
						<span class="post">총 <b>{{cnt}}</b>건의 게시물이 있습니다.</span> 
						<span class="page">(<b>1</b>/{{pageCount}} 페이지)</span>
						<span @click="fnadd" class="write"><a href="javascript:;" class="write"><b>글쓰기</b></a></span>
					</div>				
					<!-- //게시판 게시물 갯수 -->
						...
						<table>
							...
							<tbody>
								...
								<tr v-else class="new" v-for="(item, index) in list">
									...
									<a href="javascript:;">
									{{item.cmTitle}} <small>[{{item.commCnt}}]</small> 
									</a>
									</td>
	...
	var app = new Vue({
		...
		methods : {
			...
			fnGetList : function() {
				...
				var nparmap = {startNum : startNum, endNum : 10, cmKind : 2};
				$.ajax({
	    	...
	    	, fnView : function(cmNo){
				...
				$.ajax({
					...
					data : nparmap,
					success : function(data) {
						self.pageChange("/sub05_02_read", {cmNo : cmNo});
						}
				});
	    	},
	     	// 소통게시판 게시글 작성
	     	fnadd : function(){ 
	     	   location.href = "/sub05_02_add"
	    	}
		},
		created : function() {
			var self = this;
